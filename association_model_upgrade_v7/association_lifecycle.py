...
class RecordEvidence:
    ...
    record_version: str
    available: bool = True
    weighted_sound_reduction_index: float | None = None
    acoustic_unit: str = ""
    test_area_m2: float | None = None
...
def wall_snapshot_uri(global_id: str, version: str, evidence_hash: str = '') -> URIRef:
    suffix = f'-{safe_id(evidence_hash[:16])}' if evidence_hash else ''
    return URIRef(f'{BASE}ifc/element/{global_id}/snapshot/{safe_id(version)}{suffix}')


def record_snapshot_uri(record_uri: str, version: str, evidence_hash: str = '') -> URIRef:
    suffix = f'-{safe_id(evidence_hash[:16])}' if evidence_hash else ''
    return URIRef(f'{record_uri}/snapshot/{safe_id(version)}{suffix}')
...
def add_wall_snapshot(g: Graph, w: WallEvidence, evidence_hash: str = '') -> URIRef:
    stable = wall_uri(w.global_id)
    snap = wall_snapshot_uri(w.global_id, w.model_version, evidence_hash)
    ...
    if evidence_hash:
        g.add((snap, MAP.evidenceHash, Literal(evidence_hash)))
...
def add_record_snapshot(g: Graph, r: RecordEvidence, evidence_hash: str = '') -> URIRef:
    stable = URIRef(r.uri)
    snap = record_snapshot_uri(r.uri, r.record_version, evidence_hash)
    g.add((stable, RDF.type, PROV.Entity))
    ...
    if r.weighted_sound_reduction_index is not None:
        g.add((snap, MAP.weightedSoundReductionIndex, Literal(r.weighted_sound_reduction_index, datatype=XSD.decimal)))
    if r.acoustic_unit:
        g.add((snap, MAP.acousticUnit, Literal(r.acoustic_unit)))
    if r.test_area_m2 is not None:
        g.add((snap, MAP.testArea_m2, Literal(r.test_area_m2, datatype=XSD.decimal)))
    if r.surface_mass_kg_m2 is not None:
        g.add((snap, MAP.surfaceMass_kg_m2, Literal(r.surface_mass_kg_m2, datatype=XSD.decimal)))
    if r.data_year:
        g.add((snap, MAP.dataYear, Literal(r.data_year)))
    if r.source_organisation:
        g.add((snap, MAP.sourceOrganisation, URIRef(r.source_organisation)))
    if r.source_reference:
        g.add((snap, MAP.sourceReference, Literal(r.source_reference)))
    if r.source_type:
        g.add((snap, MAP.sourceType, Literal(r.source_type)))
    if r.source_note:
        g.add((snap, MAP.sourceNote, Literal(r.source_note)))
    if r.component_uri:
        g.add((snap, MAP.componentResource, URIRef(r.component_uri)))
    for u in (r.derived_from or []):
        g.add((snap, PROV.wasDerivedFrom, URIRef(u)))
    for u in (r.generated_by or []):
        g.add((snap, MAP.sourceGenerationActivity, URIRef(u)))
    for i, layer in enumerate(r.layers or [], start=1):
        layer_node = URIRef(f'{snap}/layer/{i}')
        g.add((snap, MAP.hasLayerEvidence, layer_node))
        g.add((layer_node, MAP.layerPosition, Literal(layer.get('position', i), datatype=XSD.integer)))
        if layer.get('name'):
            g.add((layer_node, MAP.layerName, Literal(layer['name'])))
        if layer.get('category'):
            g.add((layer_node, MAP.layerCategory, Literal(layer['category'])))
        if layer.get('thickness_m') is not None:
            g.add((layer_node, MAP.layerThickness_m, Literal(layer['thickness_m'], datatype=XSD.decimal)))
...
def create_assertion(g: Graph, w: WallEvidence, r: RecordEvidence, *, trigger: str,
                     ...
                     wall_evidence_hash: str | None = None, record_evidence_hash: str | None = None,
                     evidence_model_version: str = '', wall_evidence_json: str = '',
                     record_evidence_json: str = '', changes: list[dict] | None = None) -> URIRef:
    ...
    wall_snap = add_wall_snapshot(g, w, wall_evidence_hash or '')
    record_snap = add_record_snapshot(g, r, record_evidence_hash or '')
    ...
    if evidence_model_version:
        g.add((assertion, MAP.evidenceModelVersion, Literal(evidence_model_version)))
    if wall_evidence_json:
        g.add((assertion, MAP.wallEvidenceJson, Literal(wall_evidence_json)))
    if record_evidence_json:
        g.add((assertion, MAP.recordEvidenceJson, Literal(record_evidence_json)))
    for idx, change in enumerate(changes or [], start=1):
        change_node = URIRef(f'{assertion}/change/{idx}')
        g.add((assertion, MAP.hasChange, change_node))
        g.add((change_node, RDF.type, MAP.ChangeEvent))
        g.add((change_node, MAP.changeCategory, Literal(str(change.get('category', 'OTHER')))))
        g.add((change_node, MAP.changedField, Literal(str(change.get('field', '')))))
        g.add((change_node, MAP.oldValue, Literal(json.dumps(change.get('old'), ensure_ascii=False, sort_keys=True))))
        g.add((change_node, MAP.newValue, Literal(json.dumps(change.get('new'), ensure_ascii=False, sort_keys=True))))
    if previous_status:
...
def current_rows(g: Graph) -> list[dict]:
    ...
    for series in g.subjects(RDF.type, MAP.MappingSeries):
        ...
        rows.append({
            ...
            'technical_resolution': ('' if g.value(assertion, MAP.technicalResolution) is None else str(g.value(assertion, MAP.technicalResolution))),
            'requires_review': ('' if g.value(assertion, MAP.requiresReview) is None else str(g.value(assertion, MAP.requiresReview))),
...
def _decode_json_literal(value):
    if value is None:
        return None
    try:
...
def history_rows(g: Graph, series: URIRef) -> list[dict]:
    ...
    for assertion in g.subjects(RDF.type, MAP.MappingAssertion):
        ...
        rows.append({
            ...
            'changes': [
                {
                    'category': str(g.value(c, MAP.changeCategory) or ''),
                    'field': str(g.value(c, MAP.changedField) or ''),
                    'old': _decode_json_literal(g.value(c, MAP.oldValue)),
                    'new': _decode_json_literal(g.value(c, MAP.newValue)),
                }
                for c in g.objects(assertion, MAP.hasChange)
            ],
        })
