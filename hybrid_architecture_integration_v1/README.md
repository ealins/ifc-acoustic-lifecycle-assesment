...
## Evidence model v2 upgrade

`run_lifecycle_evaluation.py` now fingerprints the substantive acoustic record content as well as the stable URI and version label. The record evidence hash includes:

- weighted sound reduction index and unit
- test area and surface mass
- source year, organisation, reference, type and note
- `prov:wasDerivedFrom` and source-generation activity identifiers
- described component URI
- component construction family and total thickness
- ordered layer names, categories and layer thicknesses
- explicit record version and technical availability

The IFC evidence fingerprint also includes the IFC element name in addition to GlobalId, construction family, thickness, material evidence and model version.

This means a content or provenance change is detectable even when the stable acoustic-record URI and the manually supplied record-version label remain unchanged.

### Important migration note

Because this is a richer fingerprint definition (`evidence_model_version = 2`), do not interpret a hash change against an RDF graph generated with the earlier evaluator as a real source-data revision. For the formal lifecycle experiment, start a new graph such as `association_lifecycle_final.ttl`. Keep the previous graph as development/pilot evidence.
