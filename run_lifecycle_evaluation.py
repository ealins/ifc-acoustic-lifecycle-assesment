...
def parse_record(registry_path: Path, record_uri: str, record_version: str, force_unavailable: bool) -> RecordEvidence:
    ...
    if force_unavailable or not any(g.triples((rec, None, None))):
        ...
        return RecordEvidence(record_uri, identifier, "", "unknown", None, record_version, available=False)
    ...
    if component is None:
        return RecordEvidence(record_uri, identifier, "", "unknown", None, record_version, available=True)

    assembly = str(next(g.objects(component, DCTERMS.title), Literal("")))
    construction = str(next(g.objects(component, AC.constructionType), Literal("")))
    thickness_obj = next(g.objects(component, AC.totalThickness_m), None)
    try:
        thickness = float(thickness_obj) if thickness_obj is not None else None
    except Exception:
        thickness = None
