...
def _jsonable_evidence(obj) -> dict:
    return asdict(obj)
...
def _flatten(prefix: str, value: Any, out: dict[str, Any]) -> None:
    if isinstance(value, dict):
        ...
        # Lists are treated as one ordered/canonical evidence field. This avoids
        # misleading index-level diffs when a layer/source is inserted or removed.
        out[prefix] = value
    else:
        out[prefix] = value


def change_category(field: str) -> str:
    if field == "initial_assessment":
        return "INITIAL_ASSESSMENT"
    if field.startswith("status") or field.startswith("rationale"):
        return "ASSESSMENT_STATE_CHANGE"
    if field.startswith("wall."):
        return "IFC_EVIDENCE_CHANGE"
    if field == "record.available":
        return "RESOURCE_AVAILABILITY_CHANGE"
    if field in {
        "record.weighted_sound_reduction_index", "record.acoustic_unit",
        "record.test_area_m2", "record.surface_mass_kg_m2"
    }:
        return "ACOUSTIC_CONTENT_CHANGE"
    if field in {
        "record.data_year", "record.source_organisation", "record.source_reference",
        "record.source_type", "record.source_note", "record.derived_from", "record.generated_by"
    }:
        return "PROVENANCE_CHANGE"
    if field.startswith("record."):
        return "RECORD_METADATA_CHANGE"
    return "OTHER_CHANGE"


def evidence_changes(previous_wall: dict | None, previous_record: dict | None,
                     wall: WallEvidence, record: RecordEvidence,
                     old_status: str | None, new_status: str,
                     old_rationale: str, new_rationale: str) -> list[dict]:
    if previous_wall is None and previous_record is None:
        return [{"category": "INITIAL_ASSESSMENT", "field": "initial_assessment", "old": None, "new": "created"}]
    old_flat: dict[str, Any] = {}
    new_flat: dict[str, Any] = {}
    _flatten("wall", previous_wall or {}, old_flat)
    _flatten("record", previous_record or {}, old_flat)
    _flatten("wall", _jsonable_evidence(wall), new_flat)
    _flatten("record", _jsonable_evidence(record), new_flat)
    changes: list[dict] = []
    for field in sorted(set(old_flat) | set(new_flat)):
        old = old_flat.get(field)
        new = new_flat.get(field)
        if old != new:
            changes.append({"category": change_category(field), "field": field, "old": old, "new": new})
    if old_status != new_status:
        changes.append({"category": "ASSESSMENT_STATE_CHANGE", "field": "status", "old": old_status, "new": new_status})
    if old_rationale != new_rationale:
        changes.append({"category": "ASSESSMENT_STATE_CHANGE", "field": "rationale", "old": old_rationale or None, "new": new_rationale})
    return changes


def load_previous_evidence(g: Graph, assertion) -> tuple[dict | None, dict | None]:
    if assertion is None:
        return None, None
    def parse_json_literal(pred):
        lit = g.value(assertion, pred)
        if lit is None:
            return None
        try:
            return json.loads(str(lit))
        except Exception:
            return None
    return parse_json_literal(MAP.wallEvidenceJson), parse_json_literal(MAP.recordEvidenceJson)

def retain_last_known_record_evidence(record: RecordEvidence, previous_record: dict | None) -> tuple[RecordEvidence, bool]:
    """When the external resource is unavailable, retain the last known semantic/provenance
    evidence rather than treating every inaccessible field as if it had been deleted.

    The current observation is still marked available=False, so lifecycle diffs record a
    resource-availability change. On restoration, actual content is compared against this
    retained last-known snapshot, allowing true changes during the outage to be detected.
    """
    if record.available or not previous_record:
        return record, False
    try:
        retained = RecordEvidence(**previous_record)
    except TypeError:
        # Defensive fallback for older graphs whose stored JSON may not contain all fields.
        known = {f.name for f in __import__('dataclasses').fields(RecordEvidence)}
        retained = RecordEvidence(**{k: v for k, v in previous_record.items() if k in known})
    retained.uri = record.uri
    retained.identifier = record.identifier
    retained.record_version = record.record_version
...
def main() -> None:
    ...
    previous_wall_evidence, previous_record_evidence = load_previous_evidence(g, current)

    # Availability is a property of the current observation, not evidence that all
    # previously known record fields were deleted. During an outage, carry forward
    # the last known record evidence and flip only available=False.
    record, retained_last_known = retain_last_known_record_evidence(record, previous_record_evidence)
    ...
    changes = evidence_changes(
        previous_wall_evidence, previous_record_evidence, wall, record,
        old_status, proposed_status, old_rationale, rationale
    )
    ...
    report = {
        ...
        "availability_handling": {
            "last_known_record_evidence_retained": retained_last_known,
            "note": (
                "External resource unavailable: prior successfully observed record evidence is retained for lifecycle continuity; "
                "availability is recorded separately as false." if retained_last_known else "Current record evidence was read directly from the supplied registry."
            ),
        },
        "previous": {
    ...
    if changed or args.force:
        new_a = create_assertion(
            ...
            record_evidence_hash=rh,
            evidence_model_version=EVIDENCE_MODEL_VERSION,
            wall_evidence_json=json.dumps(asdict(wall), ensure_ascii=False, sort_keys=True),
            record_evidence_json=json.dumps(asdict(record), ensure_ascii=False, sort_keys=True),
            changes=changes,
